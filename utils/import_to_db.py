...
def process_signature_files(archive_dir, sig_type, db_path=None, batch_size=None, max_workers=None):
    ...
    try:
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_manifests_sample_id ON {schema_name}.manifests (sample_id)")
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_manifests_md5 ON {schema_name}.manifests (md5)")
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_signatures_sample_id ON {schema_name}.signatures (sample_id)")
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_signatures_md5 ON {schema_name}.signatures (md5)")
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_signature_mins_sample_id ON {schema_name}.signature_mins (sample_id)")
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{schema_name}_signature_mins_md5 ON {schema_name}.signature_mins (md5)")
    except Exception as e:
...
def process_functional_profiles(func_profiles_dir, db_path="functional_profile.db"):
    ...
    try:
        conn.execute("CREATE INDEX IF NOT EXISTS idx_functional_profile_sample_id ON functional_profile.profiles (sample_id)")
...
def process_taxonomy_mapping(data_dir, db_path="functional_profile.db"):
    ...
    tables_result = conn.execute("""
        ...
        WHERE table_schema = 'taxa_profiles' AND table_name = 'profiles'
    """).fetchall()
        ...
        try:
            if file_size_mb > 100:
                ...
                if not genome_col or not taxid_col:
                    logging.warning(f"File {file_name} does not have expected taxonomy columns. Found: {list(header_df.columns)}. Skipping.")
                    continue
                ...
                if not genome_col or not taxid_col:
                    logging.warning(f"File {file_name} does not have expected taxonomy columns. Found: {list(df_taxonomy.columns)}. Skipping.")
                    continue
    ...
    for file_path in tqdm(geo_files, desc="Processing geographical location files"):
        ...
        try:
            ...
            try:
                ...
                for chunk_df in tqdm(chunk_reader, desc=f"Reading {file_name}", leave=False):
                    ...
                    try:
                        ...
                        if file_chunk_count % 10 == 0:
                            logging.info(f"Processed {file_chunk_count} chunks from {file_name}, {file_total_records} records so far")
                ...
                logging.info(f"Successfully processed {file_name}: {file_total_records} records in {file_chunk_count} chunks")
    ...
    for archive_path in tqdm(archive_files, desc="Processing archives", disable=not progress_enabled):
        ...
        try:
            ...
            if continue_on_error:
                logging.error(f"Error processing archive {archive_name}: {str(e)}", exc_info=True)
                continue
            else:
                raise
